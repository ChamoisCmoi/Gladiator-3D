...
public class deplacementMy : MonoBehaviour
{
    ...
    void Start()
    {
        // nous allons reprendre les infos sur les compétances du joueur, sachant qu'elles sont contenues dans les PlayerPrefs, et que ces données ne peuvent pas changer quand cette scène est ouverte
        currentNiveauVitesse = PlayerPrefs.GetInt("armes2")+1;
        // currentNiveauVitesseRecul n'existe pas
        currentNiveauPointsAttaques = PlayerPrefs.GetInt("armes0")+1;
        currentNiveauPV = PlayerPrefs.GetInt("armes1")+1;
        currentNiveauTempsReAttaque = PlayerPrefs.GetInt("armes3")+1;
        currentNiveauChance = PlayerPrefs.GetInt("armes4")+1;

        // nous allons pouvoir actualiser les variables
        print(competanceParNiveau[currentNiveauPV - 1].PV.ToString() + " <color=blue>"+(currentNiveauVitesse-1).ToString()+"</color>");
        vitesse = competanceParNiveau[currentNiveauVitesse-1].vitesse*2;
        vitesseRecul = competanceParNiveau[currentNiveauVitesse-1].vitesseRecul*2;
    ...
    void Update()
    {
        if (persoAnimate.IsPlaying("attack") == false && persoAnimate.IsPlaying("victory") == false)
        {
            ...
            if ((Input.GetKey(KeyCode.LeftArrow) || Input.GetKey(KeyCode.Q))) // si il tourne à gauche
            {
                ...
                this.transform.Translate(-vitesse / 2 * Time.deltaTime,0,0);
                persoAnimate.Play("walk");
    ...
    /// <summary>
    /// est appelé pour obtenir la valeur d'une compétance du joueur (comme la vitesse de marche), en échange de la compétance voulue (je veux la valeur de la marche)
    /// </summary>
    public float GetValueCompetance(competance competance)
    {
        switch (competance)
        {
            case competance.pointsAttaque:
                return competanceParNiveau[currentNiveauPointsAttaques-1].pointsAttaques;
            case competance.PV:
                return competanceParNiveau[currentNiveauPV-1].PV;
            case competance.vitesse_marche:
                return competanceParNiveau[currentNiveauVitesse-1].vitesse;
            case competance.vitesse_attaque:
                return competanceParNiveau[currentNiveauTempsReAttaque-1].tempsReAttaque;
            case competance.chance:
                return competanceParNiveau[currentNiveauChance-1].chance;
            default:
                Debug.LogWarning("pas de valeur correspondante à la demande, la valeur de 0 a été renvoyée");
                return 0;
